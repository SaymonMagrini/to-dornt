...
class TaskController {
    ...
    public function create(): Response {
        $html = $this->view->render('admin/tasks/create', ['csrf' => Csrf::token(), 'errors' => []]);
        return new Response($html);
    }
    ...
    public function store(Request $request): Response {
        ...
        if ($errors) {
            $html = $this->view->render('admin/tasks/create', ['csrf' => Csrf::token(), 'errors' => $errors, 'old' => $request->request->all()]);
    ...
    public function update(Request $request): Response {
        ...
        if ($errors) {
            $html = $this->view->render('admin/tasks/edit', ['task' => array_merge($this->repo->find((int)$data['id']), $data), 'csrf' => Csrf::token(), 'errors' => $errors]);
            return new Response($html, 422);
        }
